...
int main() {
    ...
    int shmid = shmget(SHM_KEY, 512, IPC_CREAT | 0666);  // 0 size since we're only attaching
		//------------------------------
	    	if (shmid == -1) {
    ...
    char *data = (char *)shm_ptr;   // not yet given so wont read but wont block(since its shared memory)
